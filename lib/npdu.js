...
const encodeTarget = (buffer, target) => {
  ...
  if (target.net === 0xFFFF || !target.adr) {
    ...
    // Get length of hex
    let lenHex = Math.ceil(hex.length/2);
    // Create buffer with value mac
    // Max address mac = 2^16
    // Modify 2 by lenHex but bug with len = 1
    let buffer_mac = Buffer.alloc(2)
    buffer_mac.writeUInt16BE(target.adr);
    buffer.buffer[buffer.offset++] = buffer_mac.length;
    if (buffer_mac.length > 0) {
      for (let i = 0; i < buffer_mac.length; i++) {
        console.log('hex adr i', buffer_mac[i]);
        buffer.buffer[buffer.offset++] = buffer_mac[i];
      }
    }
  }
};
