...
const debug             = require('debug')('bacnet:client');
debug.trace             = require('debug')('bacnet:client-trace');
...
class Client extends EventEmitter {
  ...
  _processServiceRequest(serviceMap, content, buffer, offset, length) {
    ...
    if (serviceHandler) {
      try {
        ...
        debug.trace(`Handled service request${id}:`, name, JSON.stringify(content));
      } catch (e) {
  ...
  _handlePdu(buffer, offset, length, header) {
    let msg;
    debug.trace('handlePdu Header: ', header);
